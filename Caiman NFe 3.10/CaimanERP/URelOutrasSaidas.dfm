object FrmRelOutrasSaidas: TFrmRelOutrasSaidas
  Left = 337
  Top = 280
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Relat'#243'rio de Outras Sa'#237'das'
  ClientHeight = 259
  ClientWidth = 421
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Segoe UI'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 421
    Height = 201
    Align = alTop
    BevelInner = bvNone
    BevelOuter = bvNone
    BevelWidth = 2
    Color = 16710131
    TabOrder = 0
    object GB2: TGroupBox
      Left = 8
      Top = 0
      Width = 401
      Height = 45
      Caption = 'Per'#237'odo'
      TabOrder = 0
      object Label1: TLabel
        Left = 184
        Top = 22
        Width = 7
        Height = 13
        Caption = 'A'
      end
      object EdtDataIni: TDateTimePicker
        Left = 48
        Top = 16
        Width = 97
        Height = 21
        Date = 37954.412703738400000000
        Time = 37954.412703738400000000
        TabOrder = 0
      end
      object EdtDataFin: TDateTimePicker
        Left = 232
        Top = 16
        Width = 97
        Height = 21
        Date = 37954.412703738400000000
        Time = 37954.412703738400000000
        TabOrder = 1
      end
    end
    object GBTipoVenda: TGroupBox
      Left = 8
      Top = 48
      Width = 401
      Height = 65
      Caption = 'Tipo de Outras Sa'#237'das'
      TabOrder = 1
      object BtnConsTipo: TsSpeedButton
        Left = 68
        Top = 16
        Width = 23
        Height = 21
        Hint = 'Consultar Tipo de Outras Sa'#237'das'
        Glyph.Data = {
          
          0625000000000000000000000000000000000000000000000000}
        ParentShowHint = False
        ShowHint = True
        OnClick = BtnConsTipoClick
        SkinData.SkinSection = 'SPEEDBUTTON'
      end
      object EdtCodTipo: TEdit
        Left = 8
        Top = 16
        Width = 57
        Height = 21
        Hint = 'Pressione F2 para Consultar Tipo de Outras Sa'#237'das'
        Alignment = taRightJustify
        Color = clSilver
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnExit = EdtCodTipoExit
        OnKeyDown = EdtCodTipoKeyDown
        OnKeyPress = EdtCodTipoKeyPress
      end
      object EdtNomeTipo: TEdit
        Left = 95
        Top = 16
        Width = 295
        Height = 21
        TabStop = False
        CharCase = ecUpperCase
        Color = clSilver
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        MaxLength = 50
        ParentFont = False
        ParentShowHint = False
        ReadOnly = True
        ShowHint = True
        TabOrder = 1
      end
      object CheckTipo: TCheckBox
        Left = 8
        Top = 40
        Width = 329
        Height = 17
        TabStop = False
        Caption = 'Todos os Tipos de Outras Sa'#237'das'
        TabOrder = 2
        OnClick = CheckTipoClick
      end
    end
    object GroupBox1: TGroupBox
      Left = 8
      Top = 120
      Width = 401
      Height = 65
      Caption = 'Cliente'
      TabOrder = 2
      object BtnConsCli: TsSpeedButton
        Left = 68
        Top = 16
        Width = 23
        Height = 21
        Hint = 'Consultar Cliente'
        Glyph.Data = {
          
          0625000000000000000000000000000000000000000000000000}
        ParentShowHint = False
        ShowHint = True
        OnClick = BtnConsCliClick
        SkinData.SkinSection = 'SPEEDBUTTON'
      end
      object EdtCodCli: TEdit
        Left = 8
        Top = 16
        Width = 57
        Height = 21
        Hint = 'Pressione F2 para Consultar Cliente'
        Alignment = taRightJustify
        Color = clSilver
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnExit = EdtCodCliExit
        OnKeyDown = EdtCodCliKeyDown
        OnKeyPress = EdtCodTipoKeyPress
      end
      object EdtNomeCli: TEdit
        Left = 95
        Top = 16
        Width = 295
        Height = 21
        TabStop = False
        CharCase = ecUpperCase
        Color = clSilver
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        MaxLength = 50
        ParentFont = False
        ParentShowHint = False
        ReadOnly = True
        ShowHint = True
        TabOrder = 1
      end
      object CheckCliente: TCheckBox
        Left = 8
        Top = 40
        Width = 321
        Height = 17
        TabStop = False
        Caption = 'Todos os Clientes'
        TabOrder = 2
        OnClick = CheckClienteClick
      end
    end
    object CheckEmp: TCheckBox
      Left = 8
      Top = 184
      Width = 153
      Height = 17
      Caption = 'Todas as Empresas'
      TabOrder = 3
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 201
    Width = 421
    Height = 39
    Align = alClient
    BevelInner = bvNone
    BevelOuter = bvNone
    BevelWidth = 2
    Color = 16710131
    TabOrder = 1
    object BtnVis: TBitBtn
      Left = 64
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Visualizar'
      TabOrder = 0
      OnClick = BtnVisClick
    end
    object BtnImp: TBitBtn
      Left = 176
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Imprimir'
      TabOrder = 1
    end
    object BtnSair: TBitBtn
      Left = 288
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Sair'
      TabOrder = 2
      OnClick = BtnSairClick
    end
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 240
    Width = 421
    Height = 19
    Panels = <>
  end
  object QRelOS: TFDQuery
    Connection = DM.IBDatabase
    Transaction = DM.IBTransaction
    SQL.Strings = (
      
        'SELECT SUM(QUANT_ITEM) QUANT, SUM(QUANT_ITEM * VALOR_ITEM) VALOR' +
        ', O.COD_ES, I.COD_PRO,  P.NOME_PRO,  P.VALOR_PRO, P.UNIDADE_SAID' +
        'A_PRO, P.MARGEM_LUCRO_PRO, T.NOME_ES'
      'FROM ITENS_OUTRAS_SAIDAS I'
      'INNER JOIN PRODUTO P'
      'ON (I.COD_PRO = P.COD_PRO)'
      'INNER JOIN OUTRAS_SAIDAS O'
      'ON (I.COD_OUTRA = O.COD_OUTRA)'
      'INNER JOIN TIPO_ES T'
      'ON (O.COD_ES = T.COD_ES)'
      
        'GROUP BY O.COD_ES, I.COD_PRO,  P.NOME_PRO, P.VALOR_PRO, P.UNIDAD' +
        'E_SAIDA_PRO, P.MARGEM_LUCRO_PRO, T.NOME_ES'
      '')
    Left = 288
    Top = 152
    object QRelOSQUANT: TBCDField
      FieldName = 'QUANT'
      DisplayFormat = ',#0.000'
      Precision = 18
      Size = 3
    end
    object QRelOSCOD_ES: TIntegerField
      FieldName = 'COD_ES'
      Required = True
    end
    object QRelOSCOD_PRO: TIntegerField
      FieldName = 'COD_PRO'
      Required = True
    end
    object QRelOSNOME_PRO: TStringField
      FieldName = 'NOME_PRO'
      Required = True
      Size = 80
    end
    object QRelOSNOME_ES: TStringField
      FieldName = 'NOME_ES'
      Required = True
      Size = 50
    end
    object QRelOSVALOR_PRO: TBCDField
      FieldName = 'VALOR_PRO'
      Required = True
      DisplayFormat = ',#0.00'
      Precision = 18
      Size = 2
    end
    object QRelOSMARGEM_LUCRO_PRO: TBCDField
      FieldName = 'MARGEM_LUCRO_PRO'
      DisplayFormat = ',#0.00'
      Precision = 18
      Size = 2
    end
    object QRelOSUNIDADE_SAIDA_PRO: TStringField
      FieldName = 'UNIDADE_SAIDA_PRO'
      Size = 4
    end
    object QRelOSVALOR: TFloatField
      FieldName = 'VALOR'
      DisplayFormat = ',#0.00'
    end
  end
end
