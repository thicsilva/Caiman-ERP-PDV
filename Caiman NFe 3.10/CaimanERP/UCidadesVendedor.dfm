object frmCidades_Vendedor: TfrmCidades_Vendedor
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Cidades atendidas pelo Vendedor'
  ClientHeight = 296
  ClientWidth = 462
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Segoe UI'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 8
    Top = 64
    Width = 42
    Height = 13
    Caption = 'Cidade :'
  end
  object sSpeedButton3: TsSpeedButton
    Left = 283
    Top = 75
    Width = 80
    Height = 31
    Caption = 'Excluir'
    Flat = True
    Glyph.Data = {
      
      0000000000000000515500004CF6000050580000000000000000}
    OnClick = sSpeedButton3Click
    SkinData.SkinSection = 'SPEEDBUTTON'
  end
  object sSpeedButton4: TsSpeedButton
    Left = 369
    Top = 75
    Width = 80
    Height = 31
    Caption = 'Incluir'
    Flat = True
    Glyph.Data = {
      
      00000000000000000001062D0FA2042C0FED0431123B00000000}
    OnClick = sSpeedButton4Click
    SkinData.SkinSection = 'SPEEDBUTTON'
  end
  object GroupBox1: TGroupBox
    Left = 8
    Top = 8
    Width = 441
    Height = 42
    Caption = ' Vendedor : '
    TabOrder = 0
    object lblVendedor: TLabel
      Left = 11
      Top = 18
      Width = 421
      Height = 13
      Alignment = taCenter
      AutoSize = False
      Caption = 'lblVendedor'
    end
  end
  object DBGrid1: TDBGrid
    Left = 8
    Top = 112
    Width = 441
    Height = 169
    DataSource = dsCidades
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Segoe UI'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'CIDADE'
        Title.Caption = ' Cidades Atendidas'
        Width = 390
        Visible = True
      end>
  end
  object edtCidade: TEdit
    Left = 8
    Top = 81
    Width = 265
    Height = 21
    TabOrder = 2
    Text = 'edtCidade'
  end
  object TRCidades: TFDTransaction
    Connection = DM.IBDatabase
    Left = 400
    Top = 16
  end
  object QCidades: TFDQuery
    Connection = DM.IBDatabase
    Transaction = TRCidades
    UpdateObject = upCidades
    SQL.Strings = (
      'select * from vendedor_cidade'
      'where COD_VEND=:COD_VEND')
    Left = 280
    Top = 32
    ParamData = <
      item
        Name = 'COD_VEND'
      end>
    object QCidadesCOD: TIntegerField
      FieldName = 'COD'
      Origin = '"VENDEDOR_CIDADE"."COD"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object QCidadesCOD_VEND: TIntegerField
      FieldName = 'COD_VEND'
      Origin = '"VENDEDOR_CIDADE"."COD_VEND"'
    end
    object QCidadesCIDADE: TStringField
      FieldName = 'CIDADE'
      Origin = '"VENDEDOR_CIDADE"."CIDADE"'
      Size = 100
    end
  end
  object dsCidades: TDataSource
    DataSet = QCidades
    Left = 368
    Top = 24
  end
  object upCidades: TFDUpdateSQL
    InsertSQL.Strings = (
      'insert into vendedor_cidade'
      '  (CIDADE, COD, COD_VEND)'
      'values'
      '  (:CIDADE, :COD, :COD_VEND)')
    ModifySQL.Strings = (
      'update vendedor_cidade'
      'set'
      '  CIDADE = :CIDADE,'
      '  COD = :COD,'
      '  COD_VEND = :COD_VEND'
      'where'
      '  COD = :OLD_COD')
    DeleteSQL.Strings = (
      'delete from vendedor_cidade'
      'where'
      '  COD = :OLD_COD')
    FetchRowSQL.Strings = (
      'Select '
      '  COD,'
      '  COD_VEND,'
      '  CIDADE'
      'from vendedor_cidade '
      'where'
      '  COD = :COD')
    Left = 224
    Top = 152
  end
end
