object FrmRelProdutosVendidos: TFrmRelProdutosVendidos
  Left = 389
  Top = 303
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Relat'#243'rio de Produtos Vendidos'
  ClientHeight = 293
  ClientWidth = 396
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Segoe UI'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 396
    Height = 233
    Align = alTop
    BevelInner = bvNone
    BevelOuter = bvNone
    BevelWidth = 2
    Color = 16710131
    TabOrder = 0
    object GB2: TGroupBox
      Left = 2
      Top = 2
      Width = 392
      Height = 47
      Align = alTop
      Caption = 'Per'#237'odo'
      TabOrder = 0
      object Label1: TLabel
        Left = 152
        Top = 24
        Width = 7
        Height = 13
        Caption = 'A'
      end
      object EdtDataIni: TDateTimePicker
        Left = 32
        Top = 18
        Width = 97
        Height = 21
        Date = 37954.412703738400000000
        Time = 37954.412703738400000000
        TabOrder = 0
      end
      object EdtDataFin: TDateTimePicker
        Left = 192
        Top = 18
        Width = 97
        Height = 21
        Date = 37954.412703738400000000
        Time = 37954.412703738400000000
        TabOrder = 1
      end
    end
    object CheckEmp: TCheckBox
      Left = 238
      Top = 138
      Width = 139
      Height = 17
      TabStop = False
      Caption = 'Todas as Empresas'
      TabOrder = 1
    end
    object GB: TGroupBox
      Left = 2
      Top = 89
      Width = 392
      Height = 49
      Align = alTop
      Caption = 'Se'#231#227'o'
      TabOrder = 2
      object BtnCons: TsSpeedButton
        Left = 61
        Top = 18
        Width = 23
        Height = 21
        Hint = 'Consultar'
        Glyph.Data = {
          
          0625000000000000000000000000000000000000000000000000}
        ParentShowHint = False
        ShowHint = True
        OnClick = BtnConsClick
        SkinData.SkinSection = 'SPEEDBUTTON'
      end
      object EdtCod: TEdit
        Left = 8
        Top = 18
        Width = 49
        Height = 21
        Hint = 'Pressione F2 para Consultar'
        Alignment = taRightJustify
        Color = clSilver
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnEnter = EdtCodEnter
        OnExit = EdtCodExit
        OnKeyDown = EdtCodKeyDown
        OnKeyPress = EdtCodKeyPress
      end
      object EdtNome: TEdit
        Left = 87
        Top = 18
        Width = 298
        Height = 21
        TabStop = False
        CharCase = ecUpperCase
        Color = clSilver
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 1
      end
    end
    object CheckTipo: TCheckBox
      Left = 6
      Top = 138
      Width = 139
      Height = 17
      TabStop = False
      Caption = 'Todas as Se'#231#245'es'
      TabOrder = 3
      OnClick = CheckTipoClick
    end
    object RdgTipo: TRadioGroup
      Left = 2
      Top = 49
      Width = 392
      Height = 40
      Align = alTop
      Columns = 2
      Items.Strings = (
        'SE'#199#195'O'
        'FABRICANTE')
      TabOrder = 4
      OnClick = RdgTipoClick
    end
    object GroupBox1: TGroupBox
      Left = 2
      Top = 165
      Width = 392
      Height = 45
      Caption = 'Operador'
      TabOrder = 5
      object Operador: TComboBox
        Left = 8
        Top = 17
        Width = 377
        Height = 21
        Style = csDropDownList
        Color = clSilver
        TabOrder = 0
      end
    end
    object todosop: TCheckBox
      Left = 6
      Top = 212
      Width = 153
      Height = 17
      TabStop = False
      Caption = 'Todos operadores'
      TabOrder = 6
      OnClick = todosopClick
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 233
    Width = 396
    Height = 41
    Align = alClient
    BevelInner = bvNone
    BevelOuter = bvNone
    BevelWidth = 2
    Color = 16710131
    TabOrder = 1
    object BtnVis: TBitBtn
      Left = 58
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Visualizar'
      TabOrder = 0
      OnClick = BtnVisClick
    end
    object BtnImp: TBitBtn
      Left = 162
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Imprimir'
      TabOrder = 1
      OnClick = BtnVisClick
    end
    object BtnSair: TBitBtn
      Left = 262
      Top = 8
      Width = 75
      Height = 25
      Caption = '&Sair'
      TabOrder = 2
      OnClick = BtnSairClick
    end
  end
  object StatusBar1: TStatusBar
    Left = 0
    Top = 274
    Width = 396
    Height = 19
    Panels = <>
  end
  object QRelProdVend: TFDQuery
    Connection = DM.IBDatabase
    Transaction = DM.IBTransaction
    SQL.Strings = (
      
        'SELECT SUM(I.QUANT) QUANT, SUM((I.QUANT * I.VALOR) - I.DESCONTO)' +
        ' TOTAL, I.COD_PRO, P.NOME_PRO, P.COD_SEC, '
      'P.COD_LAB, S.NOME_SEC, L.NOME_LAB, U.DESCRICAO'
      'FROM ITENS_VENDA I'
      'INNER JOIN VENDAS V'
      'ON (I.COD_VEN = V.COD_VEN)'
      'INNER JOIN PRODUTO P'
      'ON (I.COD_PRO = P.COD_PRO)'
      'INNER JOIN SECAO S'
      'ON (P.COD_SEC = S.COD_SEC)'
      'INNER JOIN LABORATORIO L'
      'ON (P.COD_LAB = L.COD_LAB)'
      'INNER JOIN UNIDADE_MEDIDA U'
      'ON (P.CODIGO_UNIDADE_SAIDA = U.CODIGO)')
    Left = 328
    Top = 8
    object QRelProdVendQUANT: TBCDField
      FieldName = 'QUANT'
      ProviderFlags = []
      Precision = 18
      Size = 3
    end
    object QRelProdVendTOTAL: TFMTBCDField
      FieldName = 'TOTAL'
      ProviderFlags = []
      Precision = 18
      Size = 6
    end
    object QRelProdVendCOD_PRO: TIntegerField
      FieldName = 'COD_PRO'
      Origin = '"ITENS_VENDA"."COD_PRO"'
      Required = True
    end
    object QRelProdVendNOME_PRO: TStringField
      FieldName = 'NOME_PRO'
      Origin = '"PRODUTO"."NOME_PRO"'
      Required = True
      Size = 80
    end
    object QRelProdVendCOD_SEC: TIntegerField
      FieldName = 'COD_SEC'
      Origin = '"PRODUTO"."COD_SEC"'
      Required = True
    end
    object QRelProdVendCOD_LAB: TIntegerField
      FieldName = 'COD_LAB'
      Origin = '"PRODUTO"."COD_LAB"'
      Required = True
    end
    object QRelProdVendNOME_SEC: TStringField
      FieldName = 'NOME_SEC'
      Origin = '"SECAO"."NOME_SEC"'
      Required = True
      Size = 50
    end
    object QRelProdVendNOME_LAB: TStringField
      FieldName = 'NOME_LAB'
      Origin = '"LABORATORIO"."NOME_LAB"'
      Required = True
      Size = 50
    end
    object QRelProdVendDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Origin = '"UNIDADE_MEDIDA"."DESCRICAO"'
      Required = True
      Size = 10
    end
  end
end
