...
interface
uses
  Ubibli1, SysUtils, Dialogs, ACBrSintegra, ACBrSintegraReg;
...
{   dm.Sintegra.Registro10.CNPJ := trim( sCNPJ);
   dm.Sintegra.Registro10.Inscricao := trim(sInsc );
   dm.Sintegra.Registro10.RazaoSocial := trim( sRazao );
   dm.Sintegra.Registro10.Cidade := trim( sMunicipio );
   dm.Sintegra.Registro10.Estado := trim( sUF );
   dm.Sintegra.Registro10.Telefone := trim( sFax );
   dm.Sintegra.Registro10.DataInicial := strtodate( dataini );
   dm.Sintegra.Registro10.DataFinal := strtodate( datafin );
   dm.Sintegra.Registro10.CodigoConvenio := CodConvenio;
   dm.Sintegra.Registro10.NaturezaInformacoes := CodNatureza;
   dm.Sintegra.Registro10.FinalidadeArquivo := CodFinalidade;    }
...
{   dm.Sintegra.Registro11.Endereco := trim( sEndereco );
   dm.Sintegra.Registro11.Numero := trim( sNumero );
   dm.Sintegra.Registro11.Complemento := Trim( sComplemento );
   dm.Sintegra.Registro11.Bairro := trim( sBairro );
   dm.Sintegra.Registro11.Cep := trim( sCEP );
   dm.Sintegra.Registro11.Responsavel := trim( sResponsavel );
   dm.Sintegra.Registro11.Telefone := trim( sTelefone );}
...
begin 
   ...
   if trim(ICMS_ISS) = 'S' then
      ...
      begin 
         ...
         if (trim(TipoGRP) = 'NT') or (trim(TipoGRP) = 'OUT') then
            sSitTrib:= 'N   '
         else
         ...
         if trim(TipoGRP) = 'DES' then
            ...
            AlertaCad('Verifique Grupo de ICMS: ' + TipoGRP);
      end;
...
begin 
   ...
   if trim(ICMSISS) = 'S' then
      ...
      begin 
         ...
         if ( trim(SitTrib) = 'NT' ) or (trim(SitTrib) = 'OUT')then
            sSitTrib:= 'N   '
         else
            AlertaCad('Verifique Grupo de ICMS do produto: ' + sCodPro);
      end;
