...
uses
  ...
  DBCtrls, Forms, UNovosComponentes, FireDAC.Stan.Intf, FireDAC.Stan.Option, 
...
procedure ExecuteOnExit( Formulario: TWinControl; const vClass: Integer = 0 );
function Prxcod2(Valor_chave: Integer; transaction : TFDTransaction; ibsqlCod: TFDQuery) : integer;
...
begin
   with ibsqlCod do
      begin
         ...
         close;
         sql.Clear;
         sql.Text:= 'SELECT MAX(NUMERO_NOTA_NF) as prxcod from NOTA_FISCAL' +
         ' where COD_EMP = :valor AND CODIFICACAO_FISCAL = :CF  AND SERIE_NOTA = :SERIE';
         ParamByName( 'valor' ).AsInteger := Valor_chave;
         ParamByName( 'CF' ).AsString := '55';
       ...
       0: begin
              for I := 0 to Formulario.ComponentCount - 1 do
                 if Formulario.Components[I] is TEdit then
                    if Assigned( TEdit( Formulario.Components[I] ).OnExit ) then
                       TEdit( Formulario.Components[I] ).OnExit( nil );
          end;
       1: begin
              for I := 0 to Formulario.ComponentCount - 1 do
                 if Formulario.Components[I] is TEdit then
...
function Consulta(sTabela : String; NomeEdit : UNovosComponentes.TEdit; sCampoPK : string; SCampoPesquisa: string;
         Transaction : TFDTransaction; Qry : TFDQuery) : String;
begin
   try
     try
       ...
       with Qry do
          begin
             ...
             sql.Add(' SELECT ' + sCampoPK + ', '  + SCampoPesquisa +
                     ' FROM   ' + sTabela          +
                     ' WHERE  ' + sCampoPK + ' = ' +  QuotedStr( NomeEdit.Text ));
             Open;
      ...
      // AlertaCad('Erro ao Selecionar o(a) ' + sTabela);
     end;
   finally
     Qry.Close;
   end;
...
begin
   ...
   for i := 0 to Form.ComponentCount -1 do
       begin
          if form.Components[i] is UNovosComponentes.TEdit then
             UNovosComponentes.TEdit(form.Components[i]).Clear
          else
...
procedure Excluir(form : Tform; Tabela : string; campo : UNovosComponentes.TEdit; ibsql: TFDQuery; transaction : TFDTransaction);
begin
   if KDialog('Confirma a Exclusão?', 'Exclusão', tdtPergunta ) then
      begin
         try
           try
             if transaction.Active then
